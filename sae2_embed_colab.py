...
try:
    from config import load_config, get_axis_names, validate_config
    cfg        = load_config()
    validate_config(cfg)
    AXIS_NAMES = get_axis_names(cfg)
except Exception:
    # Fallback for Colab: read axis list embedded in cache file
    AXIS_NAMES = None
...
# Read axis list embedded by step1 if config not available
if AXIS_NAMES is None:
    if isinstance(raw, dict) and "__axis_names__" in raw:
        AXIS_NAMES = raw["__axis_names__"]
    else:
        raise RuntimeError("No config.py found and no __axis_names__ in cache. Upload pipeline_config.yaml + config.py alongside the cache file.")

# Support new list format and old flat-dict format
if isinstance(raw, list):
    posts = [p for p in raw if len(p.get("rewrites", {})) == len(AXIS_NAMES)]
elif isinstance(raw, dict) and "__axis_names__" in raw:
    posts = [p for p in raw.get("posts", []) if len(p.get("rewrites", {})) == len(AXIS_NAMES)]
else:
